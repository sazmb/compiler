...
%token BOOLOP COMPOP ADDOP MULOP UNARYOP BUILTIN IF THEN ELSE END RUN READ WRITE WHILE FOREACH IN DO
%token AND OR ADD SUB CONCAT EQUAL NEQUAL GT GE LE LT INN PLUS MINUS MUL DIVIDE NOT MINUSSIGN CAST_INT CAST_REAL
...
expr: expr bool_op bool_term {currentRule="expr_1";
                              ...
                              $$->c2=$2;
                              $$->c3=$3;}
    |bool_term {currentRule="expr_2";$$=nontermnode(NEXPR);
    tracciaVar();
                $$->c1=$1;}
    ;
bool_op: OR {currentRule="bool_op_1";$$=keynode(T_OR);}
       ...
       ;
bool_term: comp_term comp_op comp_term{currentRule="bool_term_1";$$=nontermnode(NBOOLTERM);
tracciaVar();
                                        $$->c1=$1;
                                        $$->c2=$2;
                                        $$->c3=$3;}
         | comp_term{currentRule="bool_term_2";$$=nontermnode(NBOOLTERM);
        tracciaVar();
                     $$->c1=$1;}
         ;
comp_op:EQUAL  {currentRule="comp_op_1";$$=keynode(T_EQUAL);}
       ...
       |INN {currentRule="comp_op_7";$$=keynode(T_IN);}
       ;
comp_term: comp_term add_op term {currentRule="comp_term_1";$$=nontermnode(NCOMPTERM);
tracciaVar();
                                  $$->c1=$1;
                                  $$->c2=$2;
                                  $$->c3=$3;}
          | term {currentRule="comp_term_2";$$=nontermnode(NCOMPTERM);
         tracciaVar();
                 $$->c1=$1;}
          ;
add_op: PLUS {currentRule="add_op_1";$$=keynode(T_PLUS);}
      ...
      ;
term: term mul_op factor {currentRule="term_1";$$=nontermnode(NTERM);
tracciaVar();
                          $$->c2=$2;
                          $$->c3=$3;}
     |factor {currentRule="term_2";$$=nontermnode(NTERM);
     tracciaVar();
              $$->c1=$1;}
     ;
mul_op: MUL {currentRule="mul_op_1";$$=keynode(T_MUL);}
      ...
      ;
factor: unary_op factor{currentRule="factor_1";$$=nontermnode(NFACTOR);
                        $$->c1=$1;
                        $$->c2=$2;}
       | '(' expr ')' {currentRule="factor_2";$$=nontermnode(NFACTOR);
                                 $$->c1=$2;}
       | '|' expr '|' {currentRule="factor_3";$$=nontermnode(NFACTOR);
                       $$->c1=$2;}
       | lhs  {currentRule="factor_4";$$=nontermnode(NFACTOR);
              $$->c1=$1;}
       | built_in '(' expr ')' {currentRule="factor_5";
                                 $$=nontermnode(NFACTOR);
                                 $$->c1=$1;
                                 $$->c2=$3;}
       | atomic_constant {currentRule="factor_6";$$=nontermnode(NFACTOR);
                          $$->c1=$1;}
       | record_constructor {currentRule="factor_7";$$=nontermnode(NFACTOR);
                             $$->c1=$1;}
       | array_constructor {currentRule="factor_8";$$=nontermnode(NFACTOR);
                           $$->c1=$1;}
       | cond_expr {currentRule="factor_9";$$=nontermnode(NFACTOR);
                   $$->c1=$1;}
       | func_call {currentRule="factor_10";$$=nontermnode(NFACTOR);
                    $$->c1=$1;}
       ;
unary_op: MINUSSIGN {currentRule="unary_op_1";$$=keynode(T_MINUS);}
...
atomic_constant: BOOLCONST {currentRule="atomic_constant_1";$$=boolconstnode();}
               ...
               ;
record_constructor: '}' expr_list '}' {currentRule="record_constructor_1";$$=$2;}
                                       ;
...
func_call: ID {$$=idnode();} '(' opt_expr_list ')' {currentRule="func_call_1";$$=nontermnode(NFUNCCALL);
                                                   ...
                                                   $$->c2=$4;}
         ;
opt_expr_list: expr_list {currentRule="opt_expr_list_1";$$=nontermnode(NOPTEXPRLIST);
                          $$->c1=$1;}
             | { currentRule="opt_expr_list_2";$$=NULL;}
             ;
array_constructor: '[' opt_expr_list ']' {currentRule="array_constructor_1";$$=nontermnode(NARRAYCONS);
                                          $$->c1=$2;}
                 ;
...
stat:  read_stat {currentRule="stat_1";$$=$1;}
    ...
    ;
read_stat: READ ID {currentRule="read_stat_1";$$=nontermnode(NREAD); //////////giust0???
                    $$->c1=idnode();}
         ;
...
tracciaVar();$$= nontermnode(NLHS);
                                    $$->c1=$2;
                                    $$->c2=$3;}
   | indexing {currentRule="lhs_2";$$= nontermnode(NLHS);
               $$->c1=$1;}
   ;
opt_fielding: '.' ID {$$=idnode();} {currentRule="opt_fielding_1";$$=nontermnode(NOPTFIELDING);
                      $$->c1=$3;}
...
if_stat: IF expr THEN stat_list else_part END {currentRule="if_stat_1";$$= nontermnode (NIFSTAT);
                                               ...
                                               $$->c3 =$5;}
       ;
else_part: ELSE stat_list{currentRule="else_part_1";$$=nontermnode(NELSEPART);
                          $$->c1=$2;}
         | {$$=NULL;}
         ;
