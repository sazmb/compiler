...
char *string_concat(char *buffer, char *aggiunta) {
    ...
    char *new_buffer =malloc(nuova_lunghezza);
...
void tableError(const char *messaggio, char *name) {
    ...
    // Termina il programma con uno stato di uscita non zero
    exit(3);
}
...
char *evaluate_record_format(Ttype type) {
    char *temp = string_concat(
        "{", string_concat(type->fields->name,
                               string_concat(":", evaluate_simple_format(type->fields->domain))));
    for (Param p = type->fields->next; p != NULL; p = p->next)
        temp = string_concat(
            temp, string_concat(
                ",", string_concat(p->name, string_concat(":", evaluate_simple_format(p->domain)))));
    ...
    switch (type->domain) {
        ...
        case D_ARRAY:
            if (type->child != D_RECORD)
                return string_concat("[", string_concat(evaluate_simple_format(type->child), "]"));
            if (type->child == D_RECORD) {
                return string_concat("[", string_concat(evaluate_record_format(type), "]"));
            }
        case D_RECORD:
...
void upgrade_innested_foreach() {
    ...
    if (innestedForeach_max == 0) {
        ...
        t->formals = createEntry(string_concat("index_", buffer), PARAM_CLASS, 4, create_type(D_INT, D_EMPTY),
                                 oid++,NULL);
        ...
        char *buffer = malloc(12);
        sprintf(buffer, "%d", innestedForeach_max);
        foreach_tail->next = createEntry(string_concat("index_", buffer), PARAM_CLASS, 4,
                                         create_type(D_INT, D_EMPTY), oid++,NULL);
...
Ptable createFields(Pnode node, char *func_name) {
    ...
    for (Pnode n = node->b; n != NULL; n = n->b) {
        ...
        if (temp_table == NULL || (temp_table->tclass != PARAM_CLASS)) {
            ...
            pTail->next = t;
            pTail = pTail->next;
        } else tableError("there are two parameter with the same name in function ", func_name);
    }
    reset_env();
    return pHead;
}
    ...
    switch (node->symbol) {
        ...
        case T_ID:
            ...
            if (table == NULL || table->tclass != TYPE_CLASS)
                tableError("type not yet defined", node->value.sval);
            else if (complexdomain == 1 && table->type->domain == D_ARRAY)
                tableError("you trying to create an array of array", node->value.sval);
            else if (complexdomain == 2 && (table->type->domain == D_ARRAY || table->type->domain == D_RECORD))
                tableError("you are trying to create an complex record", node->value.sval);
            else t = table->type;
            return t;
        ...
        case NARRAYTYPE: t->domain = D_ARRAY;
            if (node->c1 == NULL) {
                ...
                Ttype t2 = select_type(node->c1, 1);
                t->child = t2->domain;
                if (t->child == D_RECORD)
                    t->fields = t2->fields;
                return t;
...
int typecmp(Ttype type_a, Ttype type_b) {
    int i = 1;
    //controlla se type_a (atomic domain) è compatibile con un array
    ...
    if (type_a->domain == D_RECORD && type_b->domain == D_RECORD && (
            type_a->child != D_RECORD && type_b->child != D_RECORD)) {
        for (Param p1 = type_a->fields, p2 = type_b->fields;
             p1 != NULL && p2 != NULL;
             p1 = p1->next, p2 = p2->next)
            if ((p1->next == NULL && p2->next != NULL) || (p1->next != NULL && p2->next == NULL)
                || (strcmp(p1->name, p2->name) != 0))
                i = 0;
    } else if (type_a->domain == D_RECORD && type_b->domain == D_RECORD && (
                   type_a->child != D_RECORD || type_b->child != D_RECORD)) {
        for (Param p1 = type_a->fields, p2 = type_b->fields;
             p1 != NULL && p2 != NULL;
             p1 = p1->next, p2 = p2->next)
...
Ttype search_param_type(Ttype type, char *param_name) {
    ...
    tableError("this param not exisist in this RECORD ", param_name);
    return NULL;
}
...
void check_nfundecl(Pnode root) {
    ...
    } else tableError("this name func already declared", root->value.sval);
}

void check_nreadstat(Pnode root) {
    if (lookup(root->c1->value.sval) == NULL)
        tableError("this name is not declared (READSTAT)", root->c1->value.sval);
}

void check_nassignstat(Pnode root) {
    Ttype type = checkTree(root->c1);
    Ttype typ1 = save_type(type);
    free(type);
    Ttype typ2 = checkTree(root->c2);
    if (typecmp(typ1, typ2) == 0 &&
        !((typ1->domain == D_INT && typ2->domain == D_REAL) ||
...
Ttype check_nlhs(Pnode root) {
    ...
    if (root->c1->symbol == T_ID) {
       ...
       else if (!(table->tclass==VAR_CLASS || table->tclass==PARAM_CLASS))
            tableError(" this name is not a variable (NHLSSTAT)", root->c1->value.sval);
        ...
        if (table == NULL)
            ...
            tableError("this name is not an ARRAY (NHLSSTAT)", root->c1->value.sval);
        else if (root->c3 == NULL) {
            //caso di array di atomic
            Ttype typ1 = checkTree(root->c2);
            if (typ1->domain != D_INT)
                tableError("invalid type of expr to access to ARRAY (NHLSSTAT)", root->c1->value.sval);
            Ttype new_type=create_type(table->type->child, D_EMPTY);
            return save_type(new_type);
        } else{
           //caso di array di record
            return save_type(search_param_type(table->type, root->c3->value.sval));
            }
    }
    return NULL;
}
...
Ttype check_ncompexpr(Pnode root) {
    ...
    if (root->value.ival != T_IN) {
        ...
        } else if (type1->domain == typ2->domain && type1->domain == D_INT || type1->domain == D_REAL || type1->domain
                   == D_STRING) {
            type1->domain = D_BOOL;
            type1->child = D_EMPTY;
            type1->fields = NULL;
...
Ttype check_nmathexpr(Pnode root) {
    if (root->value.ival != T_CONCAT) {
        ...
        if ((typ1->domain == D_INT || typ1->domain == D_REAL) &&
            (typ2->domain == D_INT || typ2->domain == D_REAL)) {
                return typ1;
        ...
        free(type);
        Ttype typ2 = checkTree(root->c2);
        if (typ1->domain == D_ARRAY && typ2->domain == D_ARRAY && (
                typecmp(typ1, typ2) > 0 || typ2->child == D_EMPTY)) {
            return typ1;
        }
        tableError("mismatch in NMATHEXPR",NULL);
    }
...
Ttype check_nfuncall(Pnode root) {
    ...
    else if (table_func->tclass != FUNCT_CLASS)
        tableError("this name is not a function", root->value.sval);
